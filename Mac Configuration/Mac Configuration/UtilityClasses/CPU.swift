...
struct CpuLoadInfo {
    ...
    let totalSystemTime:UInt32 ;
    let totalUserTime:UInt32 ;
    let totalIdleTime:UInt32 ;
...
class CPU {
    ...
    func cpuLoad() -> CpuLoadInfo {
        ...
        let loadInfo:[processor_cpu_load_info] = ticks()
        var system:UInt32 = 0, user:UInt32 = 0, idle:UInt32 = 0;

        for eachProcess in loadInfo {
            
            system = eachProcess.cpu_ticks.1;
            user = eachProcess.cpu_ticks.0 + eachProcess.cpu_ticks.3;
            idle = eachProcess.cpu_ticks.2;

//            let percentLoad = 100.00 - (Double(idle)/Double(system+user+idle)) * 100.00;
//            print("\(eachProcess): \(percentLoad)")
        }
    
        return CpuLoadInfo.init(totalSystemTime: system, totalUserTime: user, totalIdleTime: idle);
    }
    
    func cpuLoadPercent() -> Double {
        
        let loadInfo = cpuLoad()
        
        let totalTime = loadInfo.totalIdleTime + loadInfo.totalSystemTime + loadInfo.totalUserTime;
        let ideal = loadInfo.totalIdleTime
        let percentLoad = 100.00 - (Double(ideal)/Double(totalTime)) * 100.00;
        return percentLoad;
